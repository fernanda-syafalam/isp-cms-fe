...
function HeaderActions({ primaryAction, secondaryActions, stickyOnMobile }: HeaderActionsProps) {
  ...
  const inline = isMobile ? [] : secondaryActions.slice(0, INLINE_SECONDARY_LIMIT)
  const overflow = isMobile ? secondaryActions : secondaryActions.slice(INLINE_SECONDARY_LIMIT)
...
function ActionMenu({ actions, label }: { actions: PageHeaderAction[]; label: string }) {
  return (
    <DropdownMenu>
