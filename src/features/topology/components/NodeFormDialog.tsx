...
export function NodeFormDialog({
  ...
  const form = useForm<NodeFormValues>({
    ...
    values: {
      ...
      splitterRatio: SplitterRatioSchema.catch("1:8").parse(
        node?.meta?.splitter,
      ),
  ...
  const types = NodeTypeSchema.options.filter(
    (t) => t !== "customer" || node?.type === "customer",
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      <DialogContent className="max-h-[85dvh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {isEdit
              ? "Perbarui detail node, uplink, dan koordinat."
              : "Titik infrastruktur baru."}
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
