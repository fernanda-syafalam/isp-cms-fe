...
import { createWorkOrderFromTicket, getTicket, listTicketEvents, listTickets } from './tickets'
...
describe('repair loop (P3.B.4)', () => {
  it('links the repair WO to its ticket and closes the ticket on completion', async () => {
    ...
    const ticket = items.find((t) => t.status !== 'resolved' && t.status !== 'breached')
    ...
    expect(events.some((e) => e.kind === 'workorder' && e.body.includes(wo.code))).toBe(true)
  })
})
